...
def generate_aes_iv(key):
    ...
    return md5(key + md5(key).hexdigest().encode()).hexdigest()[:AES_BLOCK_SIZE_BYTES].encode()
...
def rsa_decrypt_py3(data, rsa_priv_key_bytes):
    # Decrypt RSA with python 3
    key = RSA.import_key(rsa_priv_key_bytes)
    decryptor = PKCS1_OAEP.new(key)
    return decryptor.decrypt(b64decode(data))
